-- 0020_pytorch — the fifth land.
--
-- PYTORCH is Python with torch in its site-packages: the same interpreter,
-- the same `main.py`, the same stdio harness, the same `py:` mistake codes.
-- What is new is only a name, and three CHECK constraints that do not admit
-- it — `quests.land`, `attempts.lang` and `snippets.lang`. SQLite cannot
-- widen a CHECK in place, so the three tables are rebuilt the way 0008 and
-- 0017 did it: rowid carried across for the FTS5 index, the three FTS
-- triggers put back verbatim, the indexes rebuilt. Foreign keys are off for
-- the run (db.rs).
--
-- `quests` is rebuilt from its 0017 shape, which is the current one: the
-- `quiz` column and the four-way `category` CHECK come with it.

-- ---------- quests ----------
CREATE TABLE quests_new (
  id            TEXT PRIMARY KEY,
  pack          TEXT NOT NULL,
  land          TEXT NOT NULL CHECK (land IN ('rust','go','cpp','python','pytorch')),
...
CREATE TABLE attempts_new (
  ...
  lang          TEXT NOT NULL CHECK (lang IN ('rust','go','cpp','python','pytorch')),
