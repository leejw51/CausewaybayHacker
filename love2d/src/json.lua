...
---
--- Ported from `CausewaybayWallet/luacli/causewaybay/json.lua`, which was the
--- cleaner of the two candidates: `CausewaybayGolang/love2d/src/json.lua`
--- collapses `null` into `nil` and has no way to say "this empty table is an
--- array", and PROTOCOL.md puts `"id": null` on every server event and an
--- object in every `payload`. Losing either distinction breaks a frame.
---
--- Changed here, against the wallet original:
---
---   * a decoded `[]` is tagged with `json.array_mt` so it re-encodes as `[]`
---     rather than silently becoming `{}` — the wallet only ever needed the
---     marker on tables it built itself;
---   * `json.array` wraps a Lua list so an outgoing empty array stays one.
---
--- The job is deliberately narrow: RFC 8259, UTF-8, no comments, no trailing
--- commas. Two shapes need care in Lua and both are explicit:
---
---   * `null` decodes to `json.null`, a unique sentinel, because `nil` in a
---     table is indistinguishable from an absent key — and PROTOCOL §2.4
---     makes "absent" and "null" mean different things.
---   * `{}` and `[]` are the same Lua value, so the array-ness is carried on
---     a metatable.
...
--- Metatable marking a table that must encode as `[]` even when empty.
json.array_mt = { __jsonarray = true, __tostring = function() return "[]" end }

--- Marker for a table that must encode as `[]` rather than `{}`.
json.empty_array = setmetatable({}, json.array_mt)

--- Tag a Lua list as a JSON array, so an empty one survives the round trip.
